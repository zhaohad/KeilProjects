...
void OLED_ShowChar(u8 x, u8 y, u8 chr) {
	...
	if (chr < ' ') {
		if (chr == '\r') {
			f8x16_addr = ENTER_F8X16;
		} else if (chr == '\n') {
			f8x16_addr = NEW_LINE_F8X16;
		} else {
			f8x16_addr = F8X16 + ('?' - ' ') * 16;
		}
	} else {
	...
	OLED_Set_Pos(x, y);
	for (i = 0; i < 8; ++i) {
		OLED_WR_Byte(f8x16_addr[i], OLED_DATA);
	}
	OLED_Set_Pos(x, y + 1);
	for (i = 0; i < 8; ++i) {
		OLED_WR_Byte(f8x16_addr[i + 8], OLED_DATA);
	}
}
...
void OLED_ShowChar8(u8 x, u8 y, u8 chr) {
	...
	if (chr < ' ') {
		if (chr == '\r') {
			f6x8_addr = ENTER_F6X8;
		} else if (chr == '\n') {
			f6x8_addr = NEW_LINE_F6X8;
		} else {
			f6x8_addr = F6x8['?' - ' '];
		}
	} else {
  ...
  for(i = 0; i < 6; i++) {
		...
		OLED_WR_Byte(f6x8_addr[i], OLED_DATA);
	}
}
