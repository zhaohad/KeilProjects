...
void I2C_WriteNbyte_DAddr(u8 daddr, u8 addr, u8 * p, u8 number) {
	EAXSFR();		/* MOVX A,@DPTR/MOVX @DPTR,A指令的操作对象为扩展SFR(XSFR) */
	Start();                                //发送起始命令
	SendData(daddr);                         //发送设备地址+写命令
	RecvACK();
	SendData(addr);                         //发送存储地址
	RecvACK();
	do
...
void I2C_ReadNbyte_DAddr(u8 daddr, u8 addr, u8 *p, u8 number)   /*  WordAddress,First Data Address,Byte lenth   */
{
	EAXSFR();		/* MOVX A,@DPTR/MOVX @DPTR,A指令的操作对象为扩展SFR(XSFR) */
	Start();                                //发送起始命令
	SendData(daddr);                         //发送设备地址+写命令
